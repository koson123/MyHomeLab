...
- [ ] Add a local LLM only as a replaceable interpretation, retrieval, summary, recommendation, multimodal-understanding, and planning component; keep execution and permissions outside the model.
- [ ] Evaluate Qwen3.8-27B as the leading future local multimodal Jarvis candidate on hardware that can support it—especially the planned AMD Ryzen AI Halo-class upgrade—while retaining smaller local fallbacks and an interchangeable provider interface.
- [ ] Pilot Agent Reach in an isolated, least-privilege retrieval service for public web/social research, link ingestion, transcripts, GitHub, RSS, and supported Reel metadata; require read-only defaults, dedicated secondary accounts for cookie-backed platforms, protected local credentials, rate/retention limits, backend health checks, and a kill switch.
- [ ] Keep Agent Reach out of posting, messaging, account modification, and direct infrastructure execution; pass retrieved material into Jarvis as untrusted evidence that still requires validation and typed capabilities.
